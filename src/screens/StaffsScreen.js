...
const StaffsScreen = ({navigation, route}) => {
  ...
  const {createStaff, staffs, updateStaff, user_info, deviceName, deviceId} =
    useStore();
  ...
  const filteredStaffs = staffs.filter(
    createFilter(STORE._id, KEYS_TO_FILTERS),
  );
  ...
  return (
    <View>
      ...
      <AppHeader
        ...
        rightComponent={
          user_info[0].no_of_cashiers === filteredStaffs.length ||
          user_info[0].no_of_cashiers < filteredStaffs.length ? (
