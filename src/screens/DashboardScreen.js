...
const DashboardScreen = ({navigation}) => {
  ...
  const _totalSubscriptionPlan = () => {
    let branchPrice = 784;
    let cashierPrice = 35;
    let productPrice = 0.02;
    let total = 0;
    let subtotal = 0;

    if (selectedYear == '1 yr') {
      subtotal =
        (no_of_branch * branchPrice +
          no_of_cashier * cashierPrice * no_of_branch +
          no_of_products * productPrice * no_of_branch) /
        10;
  ...
  const onSelectFreePlan = () => {
    ...
    let plan = {
      ...
      privilege: 'Free Plan',
      privilege_due: `${
        moment.unix(date).add(60, 'day').startOf('day') / 1000
      }`,
    };
    ...
    if (user_info.length >= 1) {
      if (selectedYear == '3 mos') {
        ...
        let plan = {
          ...
          privilege_due: `${
            moment.unix(date).add(90, 'day').startOf('day') / 1000
          }`,
        ...
        let plan = {
          privilege: 'Custom Plan',
          privilege_due: `${
            moment.unix(date).add(365, 'day').startOf('day') / 1000
          }`,
          no_of_stores: parseInt(no_of_branch),
          no_of_cashiers: parseInt(no_of_cashier),
          no_of_products: parseInt(no_of_products),
        ...
        let plan = {
          privilege: 'Custom Plan',
          privilege_due: `${
            moment.unix(date).add(30, 'day').startOf('day') / 1000
          }`,
          no_of_stores: parseInt(no_of_branch),
          no_of_cashiers: parseInt(no_of_cashier),
          no_of_products: parseInt(no_of_products),
    ...
    <View style={{flex: 1}}>
      ...
      <AlertwithChild3
        ...
        <Text style={{padding: 10, marginLeft: 10}}>
          Please fill in additinal information :
        </Text>
      ...
      <AlertwithChild3
        ...
        <Text style={{padding: 10, marginLeft: 10}}>
          Customize plan suitable to your needs
        </Text>
        <View style={{paddingHorizontal: 20}}>
          <View style={{margin: 5}}>
              ...
              <TouchableOpacity
                onPress={
                  no_of_branch < 1
                    ? () => setNoOfBranch(1)
                    : no_of_branch > 1
                    ? () => setNoOfBranch(no_of_branch - 1)
                    : null
                }
                style={[styles.btn, {backgroundColor: colors.boldGrey}]}>
                <Text
                  style={{
                    color: colors.white,
                    fontSize: 15,
              ...
              <TouchableOpacity
                onPress={
                  no_of_cashier < 1
                    ? () => setNoOfCashier(1)
                    : no_of_cashier > 1
                    ? () => setNoOfCashier(no_of_cashier - 1)
                    : null
                }
                style={[styles.btn, {backgroundColor: colors.boldGrey}]}>
                <Text
              ...
              <TouchableOpacity
                onPress={
                  no_of_products < 1
                    ? () => setNoOfProducts(1)
                    : no_of_products > 1
                    ? () => setNoOfProducts(no_of_products - 1)
                    : null
                }
                style={[styles.btn, {backgroundColor: colors.boldGrey}]}>
                <Text
          ...
          <View
            ...
            <TouchableOpacity
              onPress={() => setSelectedYear('1 mos')}
              style={
                selectedYear == '1 mos'
                  ? [styles.year_select, {borderColor: colors.compliment}]
                  : styles.year_select
              }>
              <Text style={{textAlign: 'center'}}>1 month</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => setSelectedYear('3 mos')}
              style={
                selectedYear == '3 mos'
                  ? [styles.year_select, {borderColor: colors.compliment}]
                  : styles.year_select
          ...
          <View
            ...
            <Text>
              {formatMoney(_totalSubscriptionPlan(), {
                symbol: '₱',
                precision: 2,
              })}
            </Text>
      ...
      <AlertwithChild3
        ...
        <View style={{paddingHorizontal: 20}}>
          ...
          <View style={{marginLeft: 20}}>
            ...
            <Text style={{fontWeight: '900', marginVertical: 3}}>
              1 cashier
            </Text>
            <Text style={{fontWeight: '900', marginVertical: 3}}>
              12 products
            ...
            <Text style={{fontWeight: '900', marginVertical: 3}}>
              + 1 month free subscription for first time subscriber.{' '}
            </Text>
          </View>
        </View>
      </AlertwithChild3>
            ...
            <View style={styles.xlsubgrid}>
              <View style={{flex: 1}}>
                <View
                  ...
                  <View>
                    ...
                    <View style={{marginLeft: 20}}>
                      <Text>
                        {item.privilege} |{' '}
                        {moment.unix(item.privilege_due).format('MMM DD, YYYY')}
                      </Text>
                    </View>
                  </View>
                  <View>
                ...
                <View style={styles.xlsubgrid3}>
                  <Button
                    onPress={() => {
                      setSubscriptionVisible(true), setItem(item);
                    }}
                    buttonStyle={{
          ...
          <View>
            ...
            <View>
              <View
                ...
                <Text style={{fontSize: 15, fontWeight: 'bold', flex: 3}}>
                  Total
                </Text>
                <Text style={{fontSize: 15, fontWeight: 'bold'}}>
                  {formatMoney(calculateSale(), {symbol: '₱ ', precision: 2})}
                </Text>
                <Text
                  style={{fontSize: 15, fontWeight: 'bold', marginLeft: 30}}>
                  {formatMoney(calculateProfit(), {symbol: '₱ ', precision: 2})}
                </Text>
              </View>
            </View>
          </View>
          ...
          <View style={{width: '80%', marginTop: '10%'}}>
            ...
            <View style={{marginTop: 24}}>
              <Text
                ...
                Choose a subscription plan to unlock all the functionality of
                the application
              </Text>
            </View>
          </View>
